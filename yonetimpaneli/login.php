...
define("SITE", $siteurl);
if(!empty($_SESSION["adsoyad"]) && !empty($_SESSION["id"]) && !empty($_SESSION["tarih"]) )
{
    ?>
    ...
    <div class="card-body login-card-body">
      <?php
        if($_POST)
        {
            ...
            {
              $kullanici=filter_var($_POST["kullanici"]);
              $sifre=md5(filter_var($_POST["sifre"]));
              $kontrol=$VT->verigetir("kullanicilar","WHERE kullanici=? AND sifre=?",array($kullanici,$sifre),"ORDER BY ID ASC",1);
                ...
                if($kontrol !=false)
                {
                  ...
                  $_SESSION["tarih"]=$kontrol[0]["tarih"];
                ...
                {
                  echo '<div class="alert alert-danger"> Kullanıcı Adı veya şifreniz yanlış. </div>';
                }
            ...
            {
              echo '<div class="alert alert-danger"> Boş Alan Bırakmayız. </div>';
            }
          }
